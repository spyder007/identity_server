...
import type { ApiResourceDto, SaveApiResourceDto } from "../../api/generated/types.gen";
import {
  ClaimsPanel,
  PropertiesPanel,
  ScopesPanel,
  SecretsPanel,
} from "./ApiResourceSubResources";

function makeEmptyDraft(): SaveApiResourceDto {
  return {
    name: "",
    displayName: "",
    enabled: true,
...
function toDraft(dto: ApiResourceDto): SaveApiResourceDto {
  return {
    name: dto.name ?? "",
...
export default function ApiResourceEdit() {
  ...
  const [draft, setDraft] = useState<SaveApiResourceDto>(makeEmptyDraft);
  ...
  const load = useCallback(async () => {
    if (isNew) return;
    setLoading(true);
    const r = await getApiV1ApiResourcesById({ path: { id: numericId } });
  ...
  const submit = async () => {
    ...
    try {
      if (isNew) {
        const r = await postApiV1ApiResources({ body: draft });
        if (!r.error && r.data?.id !== undefined) {
          toast.current?.show({ severity: "success", summary: "Created", detail: "API resource created.", life: 3000 });
          navigate(`/api-resources/${r.data.id}`, { replace: true });
        }
      } else {
        const r = await putApiV1ApiResourcesById({ path: { id: numericId }, body: draft });
        if (!r.error) {
  ...
  return (
    <div className="pb-24">
      ...
      <PageHeader
        icon={faServer}
        eyebrow={isNew ? "New" : "API resource"}
        title={isNew ? "Create API resource" : draft.displayName || draft.name || "Edit API resource"}
        subtitle={isNew ? "Define a protected API and the scopes it issues" : `Name: ${draft.name}`}
        actions={
      ...
      <TabView scrollable activeIndex={activeTab} onTabChange={(e) => setActiveTab(e.index)}>
          <TabPanel header="Settings">
            <div className="space-y-8">
              <Section
                ...
                description="Identity and display details for this API resource."
              >
                <FieldGrid>
                ...
                <SwitchList>
                  <SwitchRow
                    label="Enabled"
                    description="Disabled API resources cannot be requested."
          ...
          {!isNew && <TabPanel header="Scopes"><ScopesPanel apiResourceId={numericId} /></TabPanel>}
          {!isNew && <TabPanel header="User claims"><ClaimsPanel apiResourceId={numericId} /></TabPanel>}
          {!isNew && <TabPanel header="Secrets"><SecretsPanel apiResourceId={numericId} /></TabPanel>}
          {!isNew && <TabPanel header="Properties"><PropertiesPanel apiResourceId={numericId} /></TabPanel>}
        </TabView>
